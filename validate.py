...
def test(config, test_data_loader, gen, criterionMSE, epoch):
    ...
    for i, batch in enumerate(test_data_loader):
        ...
        if config.cuda:
            x = x.cuda(0)
            t = t.cuda(0)
        x = F.interpolate(x, size=256).to(device)
        t = F.interpolate(t, size=256).to(device)
...
#             allim[0, 2, :] = np.repeat(out_cloud[None, :, :], repeats=3, axis=0) * 127.5 + 127.5
#             allim[0, 3, :] = t_rgb * 127.5 + 127.5
#             allim[0, 4, :] = np.repeat(t_cloud[None, :, :], repeats=3, axis=0) * 127.5 + 127.5
#             allim = allim.transpose(0, 3, 1, 2)
#             allim = allim.reshape((h*p, w*p, c))
